...
export default function Page() {
  ...
  const searchMatches = search.trim()
    ? rows.filter(r => `${r.dayCode} ${r.dayName}`.toLowerCase().includes(search.trim().toLowerCase())).slice(0, 8)
    : []

  const filteredRows = rows.filter(r =>
    !search.trim() || `${r.dayCode} ${r.dayName}`.toLowerCase().includes(search.trim().toLowerCase())
  )
