...
export default function Page() {
  ...
  const searchMatches = search.trim()
    ? rows.filter(r => r.sourceName.toLowerCase().includes(search.trim().toLowerCase())).slice(0, 8)
    : []

  const filteredRows = rows.filter(r =>
    !search.trim() || r.sourceName.toLowerCase().includes(search.trim().toLowerCase())
  )
