...
export function VettingPage({ nav, openModal, closeModal, showToast, openModals }: PageProps) {
  ...
  return (
    <div id="page-vetting">
      ...
      <div className="pg-hdr">
        <div>
          <h1 className="text-[1.35rem] font-semibold text-g800">
            Stage 3 &middot; Application Vetting Desk
          </h1>
          <p className="text-sm text-g500 mt-1">
            Assistant Registrar reviews documents &amp; minimum standards
          </p>
        </div>
        ...
        {PIPELINE_STEPS.map((s, i) => {
          ...
          return (
            <span key={s.num} className="contents">
              {i > 0 && (
                <span className={`pip-line ${s.num <= 3 ? 'done' : ''}`} />
              )}
              <div className={`pip-step ${cls}`}>
                <span className="pip-circle">{s.num}</span>
      ...
      <div className="card">
        <div className="flex items-center justify-between flex-wrap gap-2 mb-4">
          <h2 className="text-base font-semibold text-g800">
            Vetting Queue (5 Pending)
          </h2>
          <span className="badge-amber text-xs px-2 py-0.5 rounded-full font-medium">
            Oldest: 6h ago
          </span>
        </div>
            ...
            <tbody>
              {QUEUE.map(row => (
                <tr key={row.ref}>
                  ...
                  <td>{row.programme}</td>
                  <td>
                    <span className={`badge-${row.type === 'ODL' ? 'cyan' : row.type === 'Transfer' ? 'purple' : 'blue'} text-xs px-2 py-0.5 rounded-full`}>
                      {row.type}
                    </span>
                  </td>
                  <td>{row.docs}</td>
                  ...
                  <td>
                    <span className="badge-amber text-xs px-2 py-0.5 rounded-full">
                      {row.status}
                    </span>
                  </td>
                  <td>
                    ...
                    >
                      <i className="lni lni-eye" /> Review
                    </button>
                  </td>
                </tr>
              ))}
      ...
      {showPanel && selectedApplicant && (
        <div className="card mt-4">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3 mb-5">
            <div className="flex items-center gap-3 flex-wrap">
              <h2 className="text-base font-semibold text-g800">
                Reviewing: {selectedApplicant.name} &middot; {selectedApplicant.programme}
              </h2>
          ...
          <div className="g2">
            ...
            <div>
              <h3 className="text-sm font-semibold text-g700 mb-3">
                Uploaded Documents
              </h3>
                  ...
                  return (
                    <div
                      ...
                      className={`border rounded-[10px] overflow-hidden ${
                        isMissing
                          ? 'border-clr-red-bd bg-clr-red-bg'
                          : 'border-g200 bg-white'
                      }`}
                    >
                      <div
                        className={`h-28 flex items-center justify-center ${
                          isMissing ? 'bg-clr-red-bg' : 'bg-g100'
                        }`}
                      >
                        <i
                          className={`lni ${
                            isMissing ? 'lni-warning text-red' : 'lni-files text-g400'
                          } text-3xl`}
                        />
                      </div>
                      <div className="p-3">
                        <p className={`text-sm font-medium mb-2 ${isMissing ? 'text-red-700' : 'text-g800'}`}>
                          {doc.label}
                        </p>
                        {isMissing ? (
                          ...
                          <div className="flex gap-2">
                            ...
                            <button className="btn btn-sm btn-success text-xs">
                              <i className="lni lni-checkmark" /> Verify
                            </button>
                          </div>
                        )}
                      </div>
            ...
            {/* RIGHT — Minimum Standards Compliance */}
            <div>
              <h3 className="text-sm font-semibold text-g700 mb-3">
                ...
                {CHECKLIST.map((item, i) => (
                  <div key={i} className="chk-item">
                    <i
                      className={`lni ${
                        item.result === 'pass'
                          ? 'lni-checkmark-circle text-green-600'
                          : item.result === 'fail'
                          ? 'lni-cross-circle text-red-600'
                          : 'lni-timer text-amber-600'
                      } text-lg`}
                    />
              ...
              <div className="warn-box mb-4">
                ...
                <span className="text-sm">
                  National ID document is missing. Applicant has been notified.
                </span>
              </div>
              ...
              <button
                ...
                onClick={() => openModal('student-profile-modal')}
              >
                <i className="lni lni-user" /> View Full Profile
              </button>

              <div className="grid grid-cols-3 gap-3">
                <button
                  className="btn btn-amber w-full justify-center"
                  onClick={() => showToast('Applicant placed on hold — waiting for original documents', 'amber')}
                >
                  <i className="lni lni-timer" /> Wait for Original Documents
                </button>
                <button
                  className="btn btn-success w-full justify-center"
                  onClick={() => {
                    showToast('Application approved — provisional letter issued', 'success')
                    closePanel()
                  }}
                >
                  <i className="lni lni-checkmark" /> Approve &amp; Issue Provisional Letter
                </button>
                <button
                  className="btn btn-danger w-full justify-center"
                  onClick={() => openModal('reject-modal')}
                >
                  <i className="lni lni-close" /> Reject Application
                </button>
              </div>
            </div>
          </div>
