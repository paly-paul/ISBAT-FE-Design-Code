...
const TABS: { id: Tab; label: string; icon: string }[] = [
  ...
  { id: 'qualifications', label: 'Qualifications', icon: 'lni-graduation-cap-1' },
...
export function FilingPage({ nav, openModal, closeModal, showToast, openModals }: PageProps) {
  ...
  const filtered = APPLICANTS.filter(a =>
    `${a.id} ${a.name}`.toLowerCase().includes(applicantSearch.toLowerCase())
  )
  ...
  const PIPELINE = [
    ...
    { label: 'Filing', active: true },
    { label: 'Vetting', done: false },
  ...
  return (
    <div id="page-filing">
      ...
      <div className="pg-hdr">
        ...
        <button className="btn" onClick={() => nav('payment')}>
          <i className="lni lni-arrow-left" /> Stage 1
        </button>
      </div>
        ...
        {PIPELINE.map((s, i) => (
          <div key={i} className={`pip-step${s.done ? ' done' : ''}${(s as any).active ? ' active' : ''}`}>
            <div className="pip-circle">
              {s.done ? <i className="lni lni-checkmark" /> : i + 1}
            </div>
                ...
                <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-g200 rounded-lg shadow-lg z-20 max-h-48 overflow-y-auto">
                  ...
                  {filtered.map(a => (
                    <button key={a.id} className="w-full text-left px-4 py-2 text-sm hover:bg-b50 flex justify-between" onClick={() => selectApplicant(a)}>
                      <span className="font-medium text-g800">{a.id}</span>
                      <span className="text-g500">{a.name}</span>
                    </button>
                  ))}
                </div>
              )}
      ...
      <div className="card p-0 overflow-hidden">
        ...
        <div className="flex border-b border-g200">
          {TABS.map(t => (
            <button
              key={t.id}
              className={`flex items-center gap-2 px-5 py-3 text-sm font-semibold border-b-2 transition-colors ${activeTab === t.id ? 'border-b500 text-b600 bg-b50' : 'border-transparent text-g500 hover:text-g700'}`}
        ...
        <div className="p-5">
          ...
          {activeTab === 'personal' && (
            <div>
              ...
              <div className="flex items-start gap-5 mb-5">
                <div className="file-zone w-32 h-32 flex-shrink-0">
                  <input type="file" accept="image/*" />
                  <i className="lni lni-camera-2 file-zone-icon" />
                  <p>Profile Photo</p>
                </div>
                <div className="flex-1">
                  <div className="g3">
                    <Field label="First Name" req><Input placeholder="First name" /></Field>
                    <Field label="Middle Name"><Input placeholder="Middle name" /></Field>
                    <Field label="Last Name" req><Input placeholder="Last name" /></Field>
                  </div>
                  <div className="g3 mt-3">
                    <Field label="Gender" req><Select options={GENDERS} /></Field>
                    <Field label="Date of Birth" req><Input type="date" /></Field>
                    <Field label="Nationality" req><Select options={NATIONALITIES} /></Field>
                  </div>
                </div>
              </div>

              <div className="g3">
                <Field label="National ID"><Input placeholder="CM-XXXXX-XXXX" /></Field>
                <Field label="Email" req><Input type="email" placeholder="email@example.com" /></Field>
                <Field label="Phone" req><Input placeholder="+256 7XX XXX XXX" /></Field>
              </div>
              <div className="g3 mt-3">
                <Field label="Residential Address" span={2}><Input placeholder="Full address" /></Field>
                <Field label="University Email"><Input readOnly placeholder="Auto-generated" /></Field>
              </div>
              <div className="g3 mt-3">
                <Field label="Religion"><Select options={RELIGIONS} /></Field>
                <Field label="Marital Status"><Select options={MARITAL} /></Field>
                <div className="fg" />
              </div>
              ...
              <div className="g3 mt-3">
                <Field label="Passport Number"><Input placeholder="AB1234567" /></Field>
                <Field label="Passport Expiry"><Input type="date" /></Field>
                <Field label="Country of Issue"><Select options={NATIONALITIES} /></Field>
              </div>
              <div className="g3 mt-3">
                ...
                <Field label="Visa Expiry"><Input type="date" /></Field>
              </div>

              {/* Refugee Details */}
              <div className="sec-divider mt-5">Refugee Details</div>
              <div className="g3 mt-3">
                <Field label="Refugee Status"><Select options={['Not Applicable', 'Refugee', 'Asylum Seeker']} /></Field>
                <Field label="UNHCR Case Number"><Input placeholder="UNH-XXXX" /></Field>
                <Field label="Country of Origin"><Select options={NATIONALITIES} /></Field>
              </div>

              <div className="flex justify-end mt-5">
                <button className="btn" onClick={() => setActiveTab('qualifications')}>Next: Qualifications <i className="lni lni-arrow-right" /></button>
              </div>
            </div>
          )}
          ...
          {activeTab === 'qualifications' && (
            <div>
              ...
              <div className="g3 mt-3">
                <Field label="Type" req><Select options={QUAL_TYPES} /></Field>
                <Field label="Institution" req><Input placeholder="School / University" /></Field>
                <Field label="Year" req><Input type="number" placeholder="2023" /></Field>
              </div>
              <div className="g3 mt-3">
                <Field label="Index No."><Input placeholder="U0001/001" /></Field>
                <Field label="Percentage / GPA"><Input placeholder="e.g. 78% or 4.2" /></Field>
                <Field label="Duration (Years)"><Input type="number" placeholder="3" /></Field>
              </div>

              <div className="sec-divider mt-5 flex items-center justify-between">
                <span>Additional Qualifications</span>
                <button className="btn text-xs" onClick={() => setQualificationRows(r => [...r, { id: Date.now() }])}>
                  <i className="lni lni-plus" /> Add Row
                </button>
              </div>
              {qualificationRows.map((row, i) => (
                <div key={row.id} className="g3 mt-3 items-end">
                  <Field label="Type"><Select options={QUAL_TYPES} /></Field>
                  <Field label="Institution"><Input placeholder="Institution" /></Field>
                  <Field label="Year"><Input type="number" placeholder="Year" /></Field>
                </div>
              ))}

              <div className="sec-divider mt-5 flex items-center justify-between">
                <span>Work Experience</span>
                <button className="btn text-xs" onClick={() => setExperienceRows(r => [...r, { id: Date.now() }])}>
                  <i className="lni lni-plus" /> Add Entry
                </button>
              </div>
              {experienceRows.map((row) => (
                <div key={row.id} className="g3 mt-3">
                  <Field label="Organization"><Input placeholder="Company / Organization" /></Field>
                  <Field label="Role"><Input placeholder="Job title" /></Field>
                  <Field label="Duration"><Input placeholder="e.g. 2 years" /></Field>
                </div>
              ))}
          ...
          {activeTab === 'family' && (
            <div>
              ...
              <div className="g3 mt-3">
                <Field label="Full Name"><Input placeholder="Father's / Guardian's name" /></Field>
                <Field label="Phone"><Input placeholder="+256 7XX XXX XXX" /></Field>
                <Field label="Occupation"><Input placeholder="Occupation" /></Field>
              </div>
              <div className="g3 mt-3">
                <Field label="Email"><Input type="email" placeholder="email@example.com" /></Field>
                <div className="fg" /><div className="fg" />
              </div>

              <div className="sec-divider mt-5">Mother</div>
              <div className="g3 mt-3">
                <Field label="Full Name"><Input placeholder="Mother's name" /></Field>
                <Field label="Phone"><Input placeholder="+256 7XX XXX XXX" /></Field>
                <Field label="Occupation"><Input placeholder="Occupation" /></Field>
              </div>
              <div className="g3 mt-3">
              ...
              <div className="g3 mt-3">
                <Field label="Name" req><Input placeholder="Contact person" /></Field>
                <Field label="Relationship"><Input placeholder="e.g. Uncle" /></Field>
                <Field label="Phone" req><Input placeholder="+256 7XX XXX XXX" /></Field>
              </div>

              <div className="sec-divider mt-5">Family Address</div>
              <div className="g3 mt-3">
                <Field label="District"><Input placeholder="District" /></Field>
                <Field label="Sub-county"><Input placeholder="Sub-county" /></Field>
                <Field label="Village / Street"><Input placeholder="Village" /></Field>
              </div>

              <div className="sec-divider mt-5">Sponsorship Details</div>
              <div className="g3 mt-3">
                <Field label="Sponsor Type" req><Select options={SPONSOR_TYPES} /></Field>
                <Field label="Sponsor Name"><Input placeholder="Sponsor name" /></Field>
                <Field label="Sponsor Contact"><Input placeholder="Phone / Email" /></Field>
              </div>
            ...
            <div>
              <div className="grid grid-cols-3 gap-4 mb-5">
                {['Passport Photo', 'National ID Copy', 'O-Level Certificate', 'A-Level Certificate', 'Diploma Certificate', 'Recommendation Letter'].map(doc => (
                  <div key={doc} className="file-zone">
                    <input type="file" />
              ...
              <div className="checklist mt-3">
                {[
                  { label: 'Passport Photo', status: 'pending' },
                  { label: 'National ID Copy', status: 'pending' },
                  { label: 'O-Level Certificate', status: 'pending' },
                  { label: 'A-Level Certificate', status: 'pending', optional: true },
                  { label: 'Diploma Certificate', status: 'pending', optional: true },
                  { label: 'Recommendation Letter', status: 'pending', optional: true },
                ].map(item => (
                  <div key={item.label} className={`chk-item ${item.status}`}>
                    ...
                    <span className="chk-status text-xs">
                      {item.optional ? 'Optional' : 'Required'} &middot; {item.status === 'pass' ? 'Uploaded' : 'Pending'}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex justify-between mt-5">
                ...
                <button className="btn bg-b500 text-white" onClick={() => { showToast('Application submitted for vetting', 'success'); nav('vetting') }}>
