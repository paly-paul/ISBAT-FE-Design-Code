...
function ConvBar({ from, to, pct, level }: { from: string; to: string; pct: number; level: string }) {
  ...
  const bg = level === 'high' ? 'bg-clr-green' : 'bg-clr-red'
  return (
...
/* ── component ──────────────────────────────────────────────── */
export function DashboardPage({ nav, openModal, closeModal, showToast, openModals }: PageProps) {
  ...
  return (
    <div id="page-dashboard">
      ...
      <div className="pg-hdr">
        <div>
          ...
          <p className="text-sm text-g500 mt-0.5">
            Spring 2026 (20261) · Real-time overview of the admission pipeline
          </p>
        </div>
        <button className="btn btn-primary" onClick={() => nav('payment')}>
      ...
      {/* ── Admission Progress Charts ───────────────────────── */}
      <div className="card mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-base font-semibold text-g900">
              Admission Progress — Spring 2026 (20261)
            </h2>
            <p className="text-xs text-g400 mt-0.5">Pipeline analytics and source breakdown</p>
          </div>
          <span className="badge-green text-xs px-2 py-0.5 rounded-full font-medium">
            Live
          </span>
        </div>
          ...
          <div className="adm-chart-col">
              ...
              <div className="donut-legend">
                {DONUT_DATA.map((d) => (
                  <div key={d.label} className="donut-legend-item">
                    <span className="donut-swatch">
                      <svg width="10" height="10"><circle cx="5" cy="5" r="5" fill={d.color} /></svg>
                    </span>
            ...
            <div className="donut-metrics">
              <div className="donut-metric">
                <span className="donut-metric-lbl">Counsellor-driven</span>
                <span className="donut-metric-val">65%</span>
                <span className="donut-metric-sub">Direct + CRM</span>
              </div>
              <div className="donut-metric">
                <span className="donut-metric-lbl">Self-service</span>
                <span className="donut-metric-val">35%</span>
                <span className="donut-metric-sub">Online + ODel</span>
              </div>
              <div className="donut-metric">
                <span className="donut-metric-lbl">Top source</span>
                <span className="donut-metric-val">Direct</span>
                <span className="donut-metric-sub">9 applicants</span>
              </div>
            </div>
          </div>
              ...
              {RECENT_APPS.map((app) => (
                <tr key={app.ref} className="border-b border-g100 hover:bg-g50 transition-colors">
                  ...
                  <td className="py-2.5 pr-4">
                    <span className={`${srcBadge(app.src)} text-xs`}>{app.src}</span>
                  </td>
                  <td className="py-2.5 pr-4 text-g700">{app.prog}</td>
                  <td className="py-2.5 pr-4 text-g600">{app.type}</td>
                  <td className="py-2.5 pr-4">
                    <span className={app.fee === 'Paid' ? 'text-clr-green text-xs font-medium' : 'text-clr-red text-xs font-medium'}>
                      {app.fee}
                    </span>
                  </td>
                  <td className="py-2.5 pr-4">
                    <span className={`${stageBadge(app.stage)} text-xs`}>{app.stage}</span>
                  </td>
                  <td className="py-2.5">
                    <button
                      className="text-xs text-b500 hover:text-b700 font-medium"
