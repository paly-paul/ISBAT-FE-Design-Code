...
export function PaymentPage({ nav, openModal, closeModal, showToast, openModals }: PageProps) {
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="pg-hdr flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button className="btn bg-g100 text-g700 px-4 py-2 rounded-lg text-sm" onClick={() => nav('dashboard')}>
      ...
      <div className="pipeline flex items-center gap-0">
        {PIPELINE_STEPS.map((step, i) => (
          <div key={step} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium ${
            i === 0 ? 'bg-b500 text-white' : 'bg-g100 text-g500'
          }`}>
            <span className={`flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${
              i === 0 ? 'bg-white text-b600' : 'bg-g200 text-g500'
            }`}>{i + 1}</span>
            {step}
      ...
      <div className="g2 grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* LEFT column */}
        <div className="flex flex-col gap-5">
          {/* Application Type & Candidate Info */}
          <div className="card rounded-2xl bg-white border border-g200 p-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Intake" req>
                <select className="ctrl w-full" value={formData.intake} onChange={e => set('intake', e.target.value)}>
                  {INTAKES.map(v => <option key={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Application Source" req>
                <select className="ctrl w-full" value={formData.source} onChange={e => set('source', e.target.value)}>
            ...
            <div className="prof-photo-zone flex items-center gap-4 my-4 p-4 border-2 border-dashed border-g200 rounded-xl bg-g50 cursor-pointer hover:border-b400 transition-colors">
              <div className="w-16 h-16 rounded-full bg-g200 flex items-center justify-center text-g400">
                <i className="lni lni-camera text-xl" />
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="First Name" req>
                <input className="ctrl w-full" placeholder="Enter first name" value={formData.firstName} onChange={e => set('firstName', e.target.value)} />
              </Field>
              <Field label="Last Name" req>
                <input className="ctrl w-full" placeholder="Enter last name" value={formData.lastName} onChange={e => set('lastName', e.target.value)} />
              </Field>
              <Field label="Phone" req>
                <input className="ctrl w-full" placeholder="+256 7XX XXX XXX" value={formData.phone} onChange={e => set('phone', e.target.value)} />
              </Field>
              <Field label="Email">
                <input className="ctrl w-full" type="email" placeholder="candidate@email.com" value={formData.email} onChange={e => set('email', e.target.value)} />
              </Field>
              <Field label="Campus" req>
                <select className="ctrl w-full" value={formData.campus} onChange={e => set('campus', e.target.value)}>
                  <option value="">Select campus</option>
                  {CAMPUSES.map(v => <option key={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Programme" req>
                <select className="ctrl w-full" value={formData.programme} onChange={e => set('programme', e.target.value)}>
                  <option value="">Select programme</option>
                  {PROGRAMMES.map(v => <option key={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Fee Structure">
                <select className="ctrl w-full" value={formData.feeStructure} onChange={e => set('feeStructure', e.target.value)}>
                  <option value="">Select structure</option>
                  {FEE_STRUCTURES.map(v => <option key={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Semester">
                <select className="ctrl w-full" value={formData.semester} onChange={e => set('semester', e.target.value)}>
                  <option value="">Select semester</option>
                  {SEMESTERS.map(v => <option key={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Batch Time">
                <select className="ctrl w-full" value={formData.batchTime} onChange={e => set('batchTime', e.target.value)}>
                  <option value="">Select batch</option>
                  {BATCH_TIMES.map(v => <option key={v}>{v}</option>)}
                </select>
              </Field>
            </div>
          </div>
          ...
          <div className="card rounded-2xl bg-white border border-g200 p-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Exemption Type">
                <select className="ctrl w-full" value={exemptionType} onChange={e => setExemptionType(e.target.value)}>
                  {EXEMPTIONS.map(v => <option key={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Payment Method" req>
                <select className="ctrl w-full" value={paymentMethod} onChange={e => setPaymentMethod(e.target.value as 'cash' | 'bank')}>
                  {PAY_METHODS.map(v => <option key={v} value={v === 'Cash' ? 'cash' : 'bank'}>{v}</option>)}
                </select>
              </Field>
              <Field label="Receipt Book" req>
                <select className="ctrl w-full" value={formData.receiptBook} onChange={e => set('receiptBook', e.target.value)}>
                  <option value="">Select book</option>
                  {RECEIPT_BOOKS.map(v => <option key={v}>{v}</option>)}
                </select>
              </Field>
              <Field label="Receipt Type">
                <select className="ctrl w-full" value={formData.receiptType} onChange={e => set('receiptType', e.target.value)}>
                  <option value="">Select type</option>
                  {RECEIPT_TYPES.map(v => <option key={v}>{v}</option>)}
                </select>
              </Field>
              ...
              <Field label="Receipt / Reference No." req>
                <input className="ctrl w-full" placeholder="e.g. RCT-000123" value={formData.receiptNo} onChange={e => set('receiptNo', e.target.value)} />
              </Field>
              <Field label="Payment Date" req>
                <input className="ctrl w-full" type="date" value={formData.paymentDate} onChange={e => set('paymentDate', e.target.value)} />
              </Field>
            </div>
              ...
              <div className="mt-4 p-4 rounded-xl bg-g50 border border-g200">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Bank Name" req>
                    <select className="ctrl w-full" value={formData.bankName} onChange={e => set('bankName', e.target.value)}>
                      <option value="">Select bank</option>
                      {BANKS.map(v => <option key={v}>{v}</option>)}
                    </select>
                  </Field>
                  <Field label="Bank Transaction Ref" req>
                    <input className="ctrl w-full" placeholder="TXN-XXXXXXX" value={formData.bankRef} onChange={e => set('bankRef', e.target.value)} />
                  </Field>
                  <div className="col-span-2">
                    ...
                    <div className="flex items-center gap-3 p-3 border-2 border-dashed border-g200 rounded-lg bg-white cursor-pointer hover:border-b400 transition-colors">
                      <i className="lni lni-upload text-g400" />
                      <span className="text-sm text-g500">Click to upload or drag &amp; drop</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center gap-3 mt-5 pt-4 border-t border-g100">
              <button className="btn bg-g100 text-g600 px-5 py-2.5 rounded-lg text-sm" onClick={() => nav('dashboard')}>Cancel / Close</button>
              <button className="btn bg-clr-amber-bg text-clr-amber border border-clr-amber-bd px-5 py-2.5 rounded-lg text-sm" onClick={handleClear}>
                <i className="lni lni-trash-can mr-1" /> Clear
              </button>
              <button className="btn bg-b500 text-white px-5 py-2.5 rounded-lg text-sm ml-auto" onClick={handleSubmit}>
                <i className="lni lni-checkmark mr-1" /> Save Payment &amp; Generate Receipt
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="card rounded-2xl bg-white border border-g200 p-5">
            <h2 className="text-base font-semibold text-g800 mb-4">
              <i className="lni lni-eye mr-2 text-b500" />Live Application Preview
            </h2>
            <div className="flex flex-col gap-2">
              ...
              {paymentMethod === 'bank' && (
                <>
                  ...
                  <PreviewRow label="Bank Ref" value={formData.bankRef} />
                </>
              )}
          ...
          {showReceipt && (
            <div className="card rounded-2xl bg-white border border-g200 p-5">
              <h2 className="text-base font-semibold text-g800 mb-4">
                <i className="lni lni-ticket-alt mr-2 text-clr-green" />Generated Receipt
              </h2>
              <div className="border border-g200 rounded-xl p-5 bg-g50">
                ...
                <div className="mt-4 pt-3 border-t border-dashed border-g300 text-center text-xs text-g400">
                  This is a computer-generated receipt. No signature required.
                </div>
              </div>
              <div className="flex items-center gap-3 mt-4">
                <button className="btn bg-g100 text-g600 px-4 py-2 rounded-lg text-sm">
                  <i className="lni lni-printer mr-1" /> Print Receipt
                </button>
                <button className="btn bg-b500 text-white px-4 py-2 rounded-lg text-sm ml-auto" onClick={() => nav('filing')}>
                  Proceed to Filing <i className="lni lni-arrow-right ml-1" />
                </button>
              </div>
            </div>
          )}
...
function Field({ label, req, children }: { label: string; req?: boolean; children: React.ReactNode }) {
  return (
    <div className="fg flex flex-col gap-1">
      <label className="lbl text-xs font-medium text-g600">
        {label}{req && <span className="req text-red-500 ml-0.5">*</span>}
      </label>
      {children}
    </div>
  )
