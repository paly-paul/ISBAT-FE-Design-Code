...
import {
  useCreateGuildPayment,
  useUpdateGuildPayment,
  useDeleteGuildPayment,
  useGuildPaymentHistory,
  useGuildSemesterStatus,
  GuildPaymentRecord,
} from '@/hooks/finance/useGuildPayment'

// Standalone Guild Payment page — split out of the Payment Console per
// request, now wired to Guild's full CRUD + status surface (guild/*.md,
...
// own guild/ route (a system-wide admin list also exists —
// GET /guild/payment-guilds — but that backs its own separate console page,
// not this one). Same structure as its NCHE sibling (see
// nche-payment/page.tsx) — student search/profile/outstanding-balance are
// still the same Payment Console endpoints; only Guild-specific fields
// differ (bankDeposit instead of pnrNumber, no remarks).
...
// OutstandingCategoryTable — Guild rows have no ledger/semester/currency
...
function GuildOutstandingTable({ items, isLoading, isError }: { items: AllOutstandingItem[]; isLoading: boolean; isError: boolean }) {
  ...
  if (items.length === 0) return <div className="text-g400 text-center" style={{ padding: 16, fontSize: 12.5 }}>Fully settled — nothing outstanding on Guild.</div>
...
export default function GuildPaymentPage() {
  ...
  const { data: semesterStatus } = useGuildSemesterStatus(selectedApplicationGuid, !!selectedApplicationGuid, studentGuid)
  const { data: history = [], isLoading: isHistoryLoading } = useGuildPaymentHistory(studentGuid, !!studentGuid)
  ...
  const createGuildPayment = useCreateGuildPayment()
  const updateGuildPayment = useUpdateGuildPayment()
  const deleteGuildPayment = useDeleteGuildPayment()
  ...
  function startEdit(entry: GuildPaymentRecord) {
    setEditingGuid(entry.paymentGuildGuid)
    setAmount(String(entry.amount))
    setPayDate(entry.payDate.slice(0, 10))
    setBankDeposit(entry.bankDeposit ?? '')
  }

  function handleDelete(entry: GuildPaymentRecord) {
    if (!selectedApplicationGuid) return
    if (!window.confirm('Delete this Guild payment? This cannot be undone.')) return
    deleteGuildPayment.mutate(
      { paymentGuildGuid: entry.paymentGuildGuid, applicationGuid: selectedApplicationGuid, studentGuid },
      {
        onSuccess: () => {
          showToast('Payment deleted.', 'ok')
          if (editingGuid === entry.paymentGuildGuid) resetForm()
        },
  ...
  function handleSave() {
    ...
    if (editingGuid) {
      updateGuildPayment.mutate(
        { paymentGuildGuid: editingGuid, input: { amount: amt, payDate, bankDeposit: bankDeposit.trim() || null }, applicationGuid: selectedApplicationGuid, studentGuid },
        {
          onSuccess: result => {
            setSuccessModal({ title: 'Guild Payment Updated', rows: [['Amount', amt.toLocaleString()], ['Remaining Balance', result.remainingBalance.toLocaleString()]] })
            resetForm()
          },
          onError: (error: Error) => showToast(error.message || 'Failed to update Guild payment. Please try again.', 'error'),
        },
      )
      return
    }

    createGuildPayment.mutate(
      { applicationGuid: selectedApplicationGuid, studentGuid, amount: amt, payDate, bankDeposit: bankDeposit.trim() || null },
      {
        onSuccess: result => {
          setSuccessModal({
            title: 'Guild Payment Recorded',
            rows: [
        ...
        onError: (error: Error) => {
          ...
          showToast(error.message || 'Failed to save Guild payment. Please try again.', 'error')
        },
      },
    )
  }

  const isSaving = createGuildPayment.isPending || updateGuildPayment.isPending
    ...
    <>
      <div className="page active">
        <div className="pg-hdr">
          <div>
            <div className="pg-title">Guild Payment</div>
            <div className="pg-sub">Search student → view outstanding Guild balance → record payment</div>
          </div>
        ...
        {profile && (
          <>
            ...
            <div className="card">
              <div className="card-hdr">
                <div className="card-title"><span className="ctitle-icon"><i className="lni lni-dollar"></i></span> Outstanding Balance (Guild)</div>
                {semesterStatus && (
                  <span className={`badge ${statusBadgeClass(semesterStatus.status)}`}>{semesterStatus.semName} — {semesterStatus.status}</span>
                )}
              </div>
              <GuildOutstandingTable items={guildOutstanding} isLoading={isOutstandingLoading} isError={isOutstandingError} />
            </div>
            ...
            <div className="card">
              ...
              <div className="g2 mb-[14px]">
                <div className="fg">
                  ...
                  <div style={{ fontSize: 11, color: 'var(--g500)', marginTop: 4 }}>Must be a multiple of the configured Guild rate — enforced server-side.</div>
                </div>
                <div className="fg">
              ...
              <div className="flex gap-[10px] justify-end items-center">
                ...
                <button className="btn btn-primary btn-lg" disabled={isSaving} onClick={handleSave}>
                  <i className="lni lni-save"></i> {isSaving ? 'Saving…' : editingGuid ? 'Update Guild Payment' : 'Save Guild Payment'}
                </button>
              </div>
            </div>
            ...
            <div className="card">
              ...
              {isHistoryLoading ? (
                ...
                <ScrollTable className="no-sticky-col">
                  <table>
                    <thead><tr><th>Payment Date</th><th>Bank Deposit</th><th>Receipt</th><th>Amount</th><th style={{ width: 90 }}></th></tr></thead>
                    <tbody>
                      {history.map(h => (
                        <tr key={h.paymentGuildGuid}>
