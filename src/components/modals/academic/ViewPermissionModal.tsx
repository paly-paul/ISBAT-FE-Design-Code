...
// Read-only mirror of EditPermissionModal's own "Confirm Changes" summary
// step (module icon + name, comma-joined permission names, count badge) —
// same grouping, just without the wizard's editable checkbox state. The
// group's own `permissions` only carry {id, name} pairs with no module
// attribution (see the note on PermissionGroup) — module grouping is only
// possible by reconciling permissionName against the cached catalog, same
// as EditPermissionModal's seedFromGroupPermissions does for its own state.
