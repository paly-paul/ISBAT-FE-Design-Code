...
/admission/filing                  → application filing              ← real hook layer (backs /api/v1/admissions/application-filling, 16-endpoint spec; search + multi-step save: General/Qualifications/Photo/Submit — see note below)
...
/academic/programme-master         → programme builder (multi-step modal) ← real hook layer (create/list/update/delete all confirmed — create via apiPostForm/multipart to .../save-complete, list via GET ?search=, Edit prefills all 3 steps via GetFullDetails then PUTs apiPutForm to .../update-complete, Delete hits .../delete-complete; fee-line ledger wired to the real Finance Ledger master)
/academic/fee-structure            → fee structure & items               ← real hook layer (standalone page, unrelated to Programme Master's embedded fee-structure step — see note below; bottom-right Save now submits a real combined header+lines POST per structure to .../Programfee-structure/hd/save-complete — payload confirmed correct, but the dev backend currently 405s it (Allow: GET only) — see note below)
...
└── lib/
    ...
    └── api/
        ...
        │   ├── programMaster.ts   # also exports getProgramMasterFullDetails/updateProgramMasterComplete/deleteProgramMasterComplete (.../update-complete, .../delete-complete) — Update uses bracket-notation FormData keys (ProgramUnits[i][SemCode]), distinct from Create's dot-notation (ProgramUnits[i].SemCode) — see Data & API Architecture
...
| `applicationPayment.ts` | Real | Backs `/admission/payment`; `createApplicationPayment()` (`apiPostForm`, `POST /api/v1/admissions/application-payments`, multipart) plus 5 GET dropdown endpoints (`.../dropdowns/banks\|batches\|exemption-types\|fees\|payment-types`). `ProgramFeeHeadInfoDto` (the Fee Structure dropdown) is `{feeHdGuid, feeCode, feeDesc, intProgram, status, amtPer}` — an earlier guess (`feeHdName`/`amount`) rendered every option blank, fixed to the confirmed shape (`${feeDesc} (${feeCode})` label). `.../dropdowns/receipt-books` is confirmed broken (live 500, undocumented required `int category` param) — the page uses the generic `GET /api/v1/finance/receipt-books` (`lib/api/finance/receiptBook.ts`, `useReceiptBooks()`) instead, filtered to `status === 1` |
| `applicationFiling.ts` | Real | Backs `/admission/filing`; `/api/v1/admissions/application-filling/*` — `searchApplicationsForFiling` (GET, paginated), `saveGeneral`/`saveQualification`/`uploadPhoto` (`apiPostForm`, multipart), `deleteQualification` (`apiDelete`), `submitApplication` (`apiPost`, JSON `{appRefNo, declarationAccepted: true}`). `countryGuid`/`spCountryGuid` are deliberately omitted from every payload — confirmed via a real `/api/v1/users/countries` response that the backend only exposes `intCountryCode` (an int), no guid, anywhere |
