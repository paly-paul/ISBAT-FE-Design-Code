...
├── components/
│   ├── Header.tsx               # fixed top bar; displayName prop defaults to "Administrator" if not passed — only academic/layout.tsx passes the real one; Sign Out calls queryClient.clear() before onSignOut() so a new login never sees the previous user's cached menu/lists
