# POST /api/v1/finance/guild/payment-guild

**API ID:** `erp-finance-service.guild.payment-guild.create`
**Service:** erp-finance-compliance-service
**Module:** Finance
**Auth:** Bearer cookie via gateway; requires permission (TBD)

## Description
Creates a new Guild payment record for an application. Validates that the payment amount does not exceed the outstanding balance and is a multiple of the configured Guild rate.
...
| Amount vs Balance | business validation | Amount must not exceed outstanding Guild balance (calculated as rate × program semester count - paid amount) |
